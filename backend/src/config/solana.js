...
function getBackendKeypair() {
    if (!backendKeypair) {
        try {
            ...
            if (env_1.config.solana.backendKeypairPath) {
                logger_1.default.info("Loading backend keypair from file", {
                ...
                logger_1.default.info("Backend keypair loaded from file successfully", {
                    publicKey: backendKeypair.publicKey.toBase58(),
                });
            }
            // Option 2: Load from base58 private key
            else if (env_1.config.solana.backendAuthorityPrivateKey) {
                logger_1.default.info("Loading backend keypair from environment variable");
                const secretKey = bs58_1.default.decode(env_1.config.solana.backendAuthorityPrivateKey);
                backendKeypair = web3_js_1.Keypair.fromSecretKey(secretKey);
                logger_1.default.info("Backend keypair loaded from environment successfully", {
                    publicKey: backendKeypair.publicKey.toBase58(),
                });
            }
            // Neither provided - should never happen due to env validation
            else {
                throw new Error("No backend keypair configuration found. Must provide either BACKEND_KEYPAIR_PATH or BACKEND_AUTHORITY_PRIVATE_KEY");
            }
        }
        catch (error) {
            logger_1.default.error("Failed to load backend keypair", { error });
            throw new Error(`Failed to load backend keypair: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
