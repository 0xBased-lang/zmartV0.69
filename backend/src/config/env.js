...
const envSchema = joi_1.default.object({
    ...
    BACKEND_AUTHORITY_PRIVATE_KEY: joi_1.default.string()
        .optional()
    ...
    MARKET_MONITOR_ENABLED: joi_1.default.boolean()
        .default(false)
        .description("Enable Market Monitor service"),
    MARKET_MONITOR_CRON_SCHEDULE: joi_1.default.string()
        .default("*/5 * * * *")
        .description("Market Monitor cron schedule (every 5 minutes)"),
    MARKET_MONITOR_BATCH_SIZE: joi_1.default.number()
        .min(1)
        .max(100)
        .default(10)
        .description("Maximum markets to process per run"),
    MARKET_MONITOR_DEBUG: joi_1.default.boolean()
        .default(false)
        .description("Enable Market Monitor debug logging"),
    MARKET_MONITOR_DRY_RUN: joi_1.default.boolean()
        .default(false)
...
if (!envVars.BACKEND_KEYPAIR_PATH && !envVars.BACKEND_AUTHORITY_PRIVATE_KEY) {
    throw new Error('Backend keypair configuration required: ' +
...
exports.config = {
    ...
    solana: {
        ...
        backendAuthorityPrivateKey: envVars.BACKEND_AUTHORITY_PRIVATE_KEY,
    },
    supabase: {
    ...
    ipfs: {
        ...
        pinataSecretKey: envVars.PINATA_SECRET_KEY,
    ...
    marketMonitor: {
        enabled: envVars.MARKET_MONITOR_ENABLED,
        cronSchedule: envVars.MARKET_MONITOR_CRON_SCHEDULE,
        batchSize: envVars.MARKET_MONITOR_BATCH_SIZE,
        debugMode: envVars.MARKET_MONITOR_DEBUG,
        dryRun: envVars.MARKET_MONITOR_DRY_RUN,
    },
    logging: {
