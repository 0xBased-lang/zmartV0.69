...
exports.TradeEventWriter = void 0;
const events_1 = require("../types/events");
class TradeEventWriter {
    supabase;
    logger;
        ...
        try {
            ...
            if (tradeError) {
                ...
                if (tradeError.code === "23505") {
                    this.logger.debug(`[TradeWriter] Trade already indexed (tx: ${event.txSignature})`);
                    return {
                        success: true,
                        eventType: events_1.EventType.SHARES_BOUGHT,
            ...
            const { data: marketData, error: marketError } = await this.supabase.rpc("update_market_after_buy", {
            ...
            this.logger.info(`[TradeWriter] SharesBought: ${event.user} bought ${event.shares} ${event.outcome ? "YES" : "NO"} shares for ${event.cost} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.SHARES_BOUGHT,
                txSignature: event.txSignature,
        ...
        try {
            ...
            if (tradeError) {
                if (tradeError.code === "23505") {
                    ...
                    return {
                        success: true,
                        eventType: events_1.EventType.SHARES_SOLD,
                        txSignature: event.txSignature,
                        processingTime: Date.now() - startTime,
                    };
                }
            ...
            this.logger.info(`[TradeWriter] SharesSold: ${event.user} sold ${event.shares} ${event.outcome ? "YES" : "NO"} shares for ${event.proceeds} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.SHARES_SOLD,
                txSignature: event.txSignature,
     ...
     */
    async writeWinningsClaimed(event) {
        const startTime = Date.now();
        try {
            ...
            if (error) {
                if (error.code === "23505") {
                    this.logger.debug(`[TradeWriter] Claim already indexed (tx: ${event.txSignature})`);
                    return {
                        success: true,
                        eventType: events_1.EventType.WINNINGS_CLAIMED,
                        txSignature: event.txSignature,
                        processingTime: Date.now() - startTime,
                    };
                }
            ...
            this.logger.info(`[TradeWriter] WinningsClaimed: ${event.user} claimed ${event.payout} from market ${event.marketId} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.WINNINGS_CLAIMED,
                txSignature: event.txSignature,
     ...
     */
    async writeLiquidityWithdrawn(event) {
        const startTime = Date.now();
        try {
            ...
            if (error)
                throw error;
            this.logger.info(`[TradeWriter] LiquidityWithdrawn: ${event.creator} withdrew ${event.amount} from market ${event.marketId} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.LIQUIDITY_WITHDRAWN,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
     ...
     */
    fixedPointToDecimal(fixedPoint) {
     ...
     */
    async updateMarketAfterBuy(event) {
     ...
     */
    async updateMarketAfterSell(event) {
        const { data: market, error: fetchError } = await this.supabase
            .from("markets")
            .select("shares_yes, shares_no, total_volume")
            .eq("id", event.marketId)
...
exports.TradeEventWriter = TradeEventWriter;
//# sourceMappingURL=trade-writer.js.map