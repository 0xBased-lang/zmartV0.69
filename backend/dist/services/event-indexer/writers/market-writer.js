...
exports.MarketEventWriter = void 0;
const events_1 = require("../types/events");
class MarketEventWriter {
    supabase;
    logger;
     ...
     */
    async writeMarketProposed(event) {
        ...
        try {
            ...
            if (error) {
                ...
                if (error.code === "23505") {
                    this.logger.debug(`[MarketWriter] Market ${event.marketId} already exists (tx: ${event.txSignature})`);
                    return {
                        success: true,
                        eventType: events_1.EventType.MARKET_PROPOSED,
            ...
            this.logger.info(`[MarketWriter] Created market ${event.marketId} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.MARKET_PROPOSED,
                txSignature: event.txSignature,
     ...
     */
    async writeMarketApproved(event) {
        const startTime = Date.now();
        try {
            ...
            if (error)
                throw error;
            this.logger.info(`[MarketWriter] Approved market ${event.marketId} (${event.proposalLikes}/${event.proposalDislikes} votes, tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.MARKET_APPROVED,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
     ...
     */
    async writeMarketActivated(event) {
        const startTime = Date.now();
        try {
            const { data, error } = await this.supabase
                .from("markets")
            ...
            if (error)
                throw error;
            this.logger.info(`[MarketWriter] Activated market ${event.marketId} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.MARKET_ACTIVATED,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
     ...
     */
    async writeMarketResolved(event) {
        const startTime = Date.now();
        try {
            const { data, error } = await this.supabase
                .from("markets")
            ...
            if (error)
                throw error;
            this.logger.info(`[MarketWriter] Resolved market ${event.marketId} (outcome: ${event.proposedOutcome ? "YES" : "NO"}, tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.MARKET_RESOLVED,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
     ...
     */
    async writeDisputeInitiated(event) {
        const startTime = Date.now();
        try {
            const { data, error } = await this.supabase
                .from("markets")
            ...
            if (error)
                throw error;
            this.logger.info(`[MarketWriter] Dispute initiated for market ${event.marketId} (tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.DISPUTE_INITIATED,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
     ...
     */
    async writeMarketFinalized(event) {
        const startTime = Date.now();
        try {
            const { data, error } = await this.supabase
                .from("markets")
            ...
            if (error)
                throw error;
            this.logger.info(`[MarketWriter] Finalized market ${event.marketId} (outcome: ${event.finalOutcome ? "YES" : "NO"}, tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.MARKET_FINALIZED,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
     ...
     */
    async writeMarketCancelled(event) {
        const startTime = Date.now();
        try {
            const { data, error } = await this.supabase
                .from("markets")
            ...
            if (error)
                throw error;
            this.logger.info(`[MarketWriter] Cancelled market ${event.marketId} (reason: ${event.reason}, tx: ${event.txSignature})`);
            return {
                success: true,
                eventType: events_1.EventType.MARKET_CANCELLED,
                txSignature: event.txSignature,
                processingTime: Date.now() - startTime,
            };
        }
...
exports.MarketEventWriter = MarketEventWriter;
//# sourceMappingURL=market-writer.js.map